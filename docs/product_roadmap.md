...
| Thermoelasticity | FEM-integrated | steady/implicit-transient heat transfer, regional multi-material conductivity and capacity, amplitude-driven sources/ambient conditions, accepted-time temperature histories with coordinate-keyed cross-partition persistence, bounded temperature-property tables, and sequential isotropic thermal stress in 2D/3D | temperature-dependent conduction/capacity iteration, extreme-scale history storage, and monolithic two-way coupling remain |
