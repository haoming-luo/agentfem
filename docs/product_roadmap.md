...
| Cyclic cohesive fatigue | experimental native FEM lifecycle + 3D facet foundation | independent cycle coordinate; force-cycle extrema; thresholded cyclic Mode-I damage layered on the exact bilinear monotonic limit; native serial/MPI hyperelastic bulk-plus-cohesive Newton equilibrium with analytical interface tangent and strong-constraint reaction; global peak/valley/post-damage lifecycle; damage/structural-feedback/energy acceptance and automatic cycle cutback; exact landings; bulk+named-interface restart; atomic multi-interface split on one solver mesh; 3D failed area/front/COD; persistent same-surface crack identities with merge/split ancestry and pair ligaments; Paris-law postprocessing evidence that remains outside the solver | reference-point work and complete fatigue energy closure, cylinder benchmark, cross-partition bulk restart, CT validation and experimental prediction remain |
