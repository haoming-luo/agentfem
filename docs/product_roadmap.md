...
| Neo-Hookean solids | FEM-integrated | compressible displacement route plus monolithic P2/DG0 constant-pressure mixed route; 3D, 2D plane strain, and locally condensed finite-strain plane stress; automatic/fixed increments, consistent Newton tangent, cutback/rollback, positive-J acceptance; topology-preserving C3D10-to-C3D10H derivation; serial mixed affine-periodic solve and large-mesh Golden contract | one material; distributed mixed affine-periodic MPC, general explicit locking control, thin-3D cross-check, Cook convergence suite, and independent external-code comparison remain |
| Dynamic cohesive fracture | experimental V4 mechanism + V5 evidence foundation | finite-strain plane-stress Explicit, prestrain transfer, fixed-path Mode-I cohesive transaction, complete energy ledger, V0--V3 guardrails, crack/supershear/spall V4 mechanism ladder, physical-length refinement contract, portable interface trace, multi-observer crack fronts, live SENER/KED/J saved fields, pinned Science Dryad manifest, XLSX inspection, and curve/Mach-angle/field comparison contracts | V4 front speed remains above the ten-percent spatial gate; cohesive assembly is serial; publication-image coordinate mapping, distributed state identity, thin-3D/general incompressibility, and independent V5 research comparison remain |
