...
coordinate-keyed nodal state. It can be read with a different MPI partition or
rank count; a two-rank heat state is automatically tested by continuation on
one rank. The default remains the smaller rank-sharded format. Portable state
uses bounds-scaled integer coordinate keys to absorb partition-dependent mesh
construction roundoff while retaining a global connectivity identity. The
current NPZ implementation is suitable for laboratory-scale restart; a
