...
# Keep the current source tree ahead of any older installed release when
# validating imports from a checkout or unpacked source distribution.
SOURCE_ROOT = ROOT / "src"
source_root = str(SOURCE_ROOT)
if not sys.path or sys.path[0] != source_root:
    sys.path.insert(0, source_root)
KNOWLEDGE_DIR = SOURCE_ROOT / "agentfem" / "knowledge"
...
# Scientific Function Cards store TeX, not informal ASCII pseudocode.  Most
# malformed expressions are still accepted by MathJax and therefore look like
# long products of italic letters instead of producing an explicit build
# error.  Keep this deliberately small and unambiguous: it catches notation
# that has repeatedly escaped visual review without prescribing one house
# style for every valid equation.
ASCII_MATH_TOKEN_PATTERN = re.compile(
    r"(?<![\\A-Za-z])(?:Delta|delta|epsilon|beta|eta|theta|omega|alpha|kappa|"
    r"lambda|mu|sigma|Phi)(?=[^A-Za-z]|$)"
    r"|(?<![\\A-Za-z])(?:sum|max|min|exp|dot)(?=[_(\s])"
    r"|_\("
    r"|\|\|"
)


def _equation_notation_errors(expression: str) -> list[str]:
    """Return definite ASCII-pseudocode fragments in a TeX expression."""

    return sorted({match.group(0) for match in ASCII_MATH_TOKEN_PATTERN.finditer(expression)})


def _repository_path(path: str) -> Path:
    """Resolve repository assets while keeping card paths package-relative."""

    root_candidate = ROOT / path
    if root_candidate.exists():
        return root_candidate
    return SOURCE_ROOT / "agentfem" / path
...
def _validate_card(
    ...
    if isinstance(card["implementation"], list):
        for path in card["implementation"]:
            if isinstance(path, str) and not _repository_path(path).is_file():
            ...
            for index, equation in enumerate(equations):
                ...
                if row is not None:
                    ...
                    if isinstance(expression, str):
                        malformed = _equation_notation_errors(expression)
                        if malformed:
                            errors.append(
                                f"{label}.science.equations[{index}].expression "
                                "must use TeX notation instead of ASCII pseudocode; "
                                f"found {malformed}"
                            )
    ...
    if not isinstance(references, list) or not references:
        ...
        for index, reference in enumerate(references):
            ...
            if (
                ...
                and not _repository_path(locator).exists()
            ):
...
def _validate_benchmark(source: Path, benchmark: Mapping[str, object]) -> list[str]:
    ...
    if execution is not None:
        ...
        if isinstance(example, str) and not _repository_path(example).is_file():
